...
namespace Shogi.Stage
{
    ...
    public class MainStage : MonoBehaviour
    {
        ...
        /// </summary>
        [SerializeField, Tooltip("0: 歩, 1: 香車, 2: 桂馬, 3: 銀将, 4: 金将, 5: 角行, 6: 飛車, 7: 王将, 8: 玉将 となるように設定")] 
        private Piece.Piece[] _piecePrefabs;
        /// <summary>
        /// 駒の親オブジェクト
        /// </summary>
        ...
        /// 将棋盤上の駒の配列
        /// </summary>
        private Piece.Piece[,] _pieces;
        /// <summary>
        /// 歩のオブジェクト配列
        /// </summary>
        private Piece.Piece[] _fuPieces;
        /// <summary>
        /// 歩のオブジェクト配列
        /// </summary>
        public Piece.Piece[] FuPieces => _fuPieces;

        public Piece.Piece GetPieceAt(Vector2Int pos)
        {
        ...
        public IEnumerator PlacePiece(Piece.Piece piece, Vector2Int pos)
        {
        ...
        public Piece.Piece PickupPiece(Vector2Int pos)
        {
            if (_pieces[pos.x, pos.y] == null)
            {
        ...
        {
            // 配列の初期化
            _stageCells = new StageCell[Logic.Constants.BOARD_SIZE, Logic.Constants.BOARD_SIZE];

            for (int x = 0; x < Logic.Constants.BOARD_SIZE; x++)
            {
                for (int y = 0; y < Logic.Constants.BOARD_SIZE; y++)
                {
                    // マスのワールド座標を計算して生成
                    Vector3 masPos = new Vector3(
                        _cellCenterPosition.x + (x - Logic.Constants.BOARD_SIZE / 2) * _cellSpacing,
                        _cellCenterPosition.y,
                        _cellCenterPosition.z + (y - Logic.Constants.BOARD_SIZE / 2) * _cellSpacing
                    );
        ...
        {
            List<Piece.Piece> fuPieces = new List<Piece.Piece>();
            _pieces = new Piece.Piece[Logic.Constants.BOARD_SIZE, Logic.Constants.BOARD_SIZE];

            foreach (var placementInfo in initialPlacementInfos)
            {
                // 駒の生成(_piecePrefabs配列から駒名に対応するプレハブを取得)
                Piece.Piece piecePrefab = null;
        ...
        {
            if (logicPos.x < 0 || logicPos.x >= Logic.Constants.BOARD_SIZE || logicPos.y < 0 || logicPos.y >= Logic.Constants.BOARD_SIZE)
            {
        ...
        public bool TryGetPieceAt(Vector2Int logicPos, out Piece.Piece occupyingPiece)
        {
            // 範囲外チェック
            if (logicPos.x < 0 || logicPos.x >= Logic.Constants.BOARD_SIZE || logicPos.y < 0 || logicPos.y >= Logic.Constants.BOARD_SIZE)
            {
